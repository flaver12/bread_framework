<?php
/**
 * @author syz, syz <aurel@swagpeople.ch>, Flavio Kleiber <flavio@swagpeople.ch>, 
 * @copyright syz, Flavio Kleiber, swagpeople.ch (c) 2014
 *
 * Create a Bill
**/
class BillGen extends PDF {
	function __construct() {
		...
		$q =  "SELECT company, id 
			FROM  bCustomer
			WHERE id = 1";
		$Customer = $query->sendQuery($q);
		$this->registerBill($Customer[0]['id'], $total);
		$filename = date('d-m-Y');
		$filename .= '-'.$Customer[0]['company'];
		$pdf->Output(ROOT.'/weebroot/bills/'. $filename .'.pdf', 'F');
	}
}	